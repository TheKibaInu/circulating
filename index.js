...
app.get("/", async (req, res) => {
  try {
    const request = await axios.get("https://kibainu-bsc.vercel.app/");
...
app.get("/maxcoins", async (req, res) => {
  try {
    const request = await axios.get("https://kibainu-bsc.vercel.app/maxcoins");
...
app.get("/totalcoins", async (req, res) => {
  try {
    const request = await axios.get(
      "https://kibainu-bsc.vercel.app/totalcoins"
    );
    const tempData = new BigNumber(request.data);
    res.status(200).end(tempData.toFixed(maxFrac));
  } catch (e) {
...
app.get("/circulating", async (req, res) => {
  try {
    const request = await axios.get(
      "https://kibainu-bsc.vercel.app/circulating"
    );
    const tempData = new BigNumber(request.data);
    res.status(200).end(tempData.toFixed(maxFrac));
  } catch (e) {
...
app.get("/about", async (req, res) => {
  try {
    const request = await axios.get("https://kibainu-bsc.vercel.app/about");
